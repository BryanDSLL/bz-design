...
export const MacbookScroll = ({
  ...
  const [translateEnd, setTranslateEnd] = useState(1500);
  ...
  useEffect(() => {
    const updateValues = () => {
      ...
      // Scale the notebook proportionally to viewport width
  ...
  // Zero translation so it stays centered while pinned
  const translate = useTransform(scrollYProgress, [0, 1], [0, 0]);
  const rotate = useTransform(scrollYProgress, [0.1, 0.12, 0.3], [-28, -28, 0]);
  // Title stays centered
  const textTransform = useTransform(scrollYProgress, [0, 0.3], [0, 0]);
  const textOpacity = useTransform(scrollYProgress, [0, 0.2], [1, 0]);

  return (
    <div
      ref={ref}
      className="min-h-[220vh] w-full"
    >
      <div className="sticky top-0 flex h-screen w-full items-center justify-center overflow-hidden">
        <div
          className="flex flex-col items-center justify-center shrink-0 [perspective:800px]"
